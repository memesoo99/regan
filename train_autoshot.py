...
from loss.losses import calc_loss, dice_loss, threshold_predictions_v,threshold_predictions_p
from ploting import plot_kernels, LayerActivations, input_images, plot_grad_flow
from metric.Metrics import dice_coeff, accuracy_score
import time
...
model_test = model_unet(U_Net, 3, 1)
...
l_data = config['l_data']
test_image = config['test_image']
test_label = config['test_label']
test_folderP = config['test_folderP']
test_folderL = config['test_folderL']
...
read_model_path = './model_unet/Unet_D_' + str(epoch) + '_' + str(batch_size)
...
#     print("label!!",np.array(im_label).shape)
#     print(np.array(im_label))
   ...
   #pred_tb = threshold_predictions_v(pred_tb)
    ...
    x1 = plt.imsave(
        './model_unet/pred/img_iteration_' + str(n_iter) + '_epoch_'
 ...
 #       writer1.add_scalar('Train Loss', train_loss, n_iter)
  #      writer1.add_scalar('Validation Loss', valid_loss, n_iter)
    ...
    if valid_loss <= valid_loss_min and epoch_valid >= i: # and i_valid <= 2:
        ...
        torch.save(model_test.state_dict(),'./model_unet/Unet_D_' +
...
test1 =model_test.load_state_dict(torch.load('./model_unet/Unet_D_' +
                   str(epoch) + '_' + str(batch_size)+ '/Unet_epoch_' + str(epoch)
                   + '_batchsize_' + str(batch_size) + '.pth'))
...
model_test.load_state_dict(torch.load('./model_unet/Unet_D_' +
                   str(epoch) + '_' + str(batch_size)+ '/Unet_epoch_' + str(epoch)
                   + '_batchsize_' + str(batch_size) + '.pth'))
...
read_test_folder112 = './model_unet/gen_images'
...
read_test_folder_P_Thres = './model_unet/pred_threshold'
...
read_test_folder_L_Thres = './model_unet/label_threshold'
...
#    pred = threshold_predictions_p(pred) #Value kept 0.01 as max is 1 and noise is very small.
    ...
    x1 = plt.imsave('./model_unet/gen_images/im_epoch_' + str(epoch) + 'int_' + str(i)
...
read_test_folderP = glob.glob('./model_unet/gen_images/*')
...
for i in range(len(read_test_folderP)):
    ...
    #save the images
    x1 = plt.imsave('./model_unet/pred_threshold/im_epoch_' + str(epoch) + 'int_' + str(i)
   ...
   # s2 =threshold_predictions_v(s2)
    ...
    #save the Images
    y1 = plt.imsave('./model_unet/label_threshold/im_epoch_' + str(epoch) + 'int_' + str(i)
